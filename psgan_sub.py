...
def create_generator(generator_inputs1, generator_inputs2, generator_outputs_channels):
    ...
    with tf.variable_scope("encoder_1_1"):
        convolved = conv(generator_inputs1, 3, 32, 1)
...
def load_examples():
    ...
    features = tf.parse_single_example(serialized_example,
                                       features={
                                           ...
                                           'im_lr_u_raw': tf.FixedLenFeature([], tf.string),
    ...
    im_lr_raw = tf.decode_raw(features['im_lr_raw'], tf.uint8)
    im_lr_raw = tf.reshape(im_lr_raw, [32, 32, 4])
    im_lr_raw=tf.cast(im_lr_raw, tf.float32)
    ...
    if a.mode == 'train':
        inputs1_batch, inputs2_batch, targets_batch = tf.train.shuffle_batch([im_lr_raw, im_pan_raw, im_mul_raw],
                                              batch_size=a.batch_size, capacity=200,
                                              min_after_dequeue=100)
        steps_per_epoch = int(a.train_count / a.batch_size)
    elif a.mode =='test':
        inputs1_batch, inputs2_batch, targets_batch = tf.train.batch([im_lr_raw, im_pan_raw, im_mul_raw],
                                              batch_size=a.batch_size, capacity=200)
